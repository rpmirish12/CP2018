...
def linearWeight(pixel_value):
    ...
    """
    z_min, z_max = 0., 255.
    condition = (z_min+z_max)/2
    # set the pixel value according to the weighting defined in the comments above
    if pixel_value <= condition:
...
def sampleIntensities(images):
    ...
    for intensity in xrange(0, 256): # Iterate through each possible pixel intensity

        # grab the positions of the matching intensities of the image
        # https://stackoverflow.com/questions/34667282/numpy-where-detailed-step-by-step-explanation-examples
        rows, cols = np.where(mid_img == intensity)
        num_candidate_locations = len(rows) # number of identified matches

        # as noted above if there are no pixels in IM with value Zi, do nothing
        if num_candidate_locations != 0:

            # select a random index given total number of matches
            # https://docs.scipy.org/doc/numpy/reference/generated/numpy.random.randint.html 
            random_selector = random.randint(0, num_candidate_locations-1)

            # iterate through the images and set the intensity values
            for image in xrange(num_images):
                tmp_img = images[image] # set the image
                row = rows[random_selector] # calc the row index
                col = cols[random_selector] # calc the col index
                intensity_values[intensity][image] = tmp_img[row][col] # store the intensity value
...
def computeResponseCurve(intensity_samples, log_exposures, smoothing_lambda, weighting_function):
    ...
    mat_b = np.zeros((mat_A.shape[0], 1), dtype=np.float64)
     ...
     # https://piazza.com/class/jc55krnjbsgm9?cid=825 Hugely helpful!
    ...
    k = 0 # set k for overall row position as noted in comments above

    # iterate through the samples and images and calculate data constraints
    for sample in xrange(num_samples):
        for image in xrange(num_images):

            # grab the pixel intensity and apply the weight function to it
            z = intensity_samples[sample, image]
            w = weighting_function(z)

            # follow the steps above for operations on mat_A and mat_B
            mat_A[k][z] = w
            mat_A[k][num_samples + sample] = -w
            mat_b[k] = w * log_exposures[image]
    ...
    # iterate through the intensity values to calculate smoothing constraints
    # as noted in the comments above
    for z_k in xrange(1, 255):
        w_k = weighting_function(z_k)

        # set mat_A and mat_B values as noted for each row/col from the notes above
        mat_A[k][z_k-1] = w_k * smoothing_lambda
        mat_A[k][z_k] = -2 * w_k * smoothing_lambda
        mat_A[k][z_k+1] = w_k * smoothing_lambda

        # shift to the next overall row
        k = k + 1
    ...
    #
    # set the value of mat_A as noted above
    mat_A[k][(255-0) // 2] = 1
...
def computeRadianceMap(images, log_exposure_times, response_curve, weighting_function):
    ...
    # set the rows/cols for iteration and the total number of images
    rows, cols = img_shape
    num_images = len(images)

    # instantiate np arrays for g_z and w_ij
    g_z = np.zeros(num_images, dtype = np.float64)
    w_ij = np.zeros(num_images, dtype = np.float64)

    # set the middle image index for use in the final step
    mid_img = num_images // 2

    # following the algorithm outlined in the above comment, iterate through
    # all of the pixel values and set the new combined output img_rad_map
    for row in xrange(rows):
        for col in xrange(cols):

            # iterate throught the images and set their pixel instensity, irradiance function
            # and weighting function values for use in the equations noted above
            for image in xrange(num_images):
                tmp_img = images[image] # grabs the image
                z_ij = tmp_img[row,col] # gets the pixel instensity

                # set the responsive curve and weight values into the relevant arrays
                g_z[image] = response_curve[z_ij]
                w_ij[image] = weighting_function(z_ij)

            # sum the weights for all values as noted above
            sum_w = np.sum(w_ij)

            # implementing the directions above to set the values if there is a pixel 
            # in the responsive curve, if not, then set the according to the middle image
            if sum_w > 0:
                img_rad_map[row][col] = np.sum(w_ij * (g_z - log_exposure_times) / sum_w)
            else:
                img_rad_map[row][col] = g_z[mid_img] - log_exposure_times[mid_img]

    # return the radiance map of the images            
    return img_rad_map